...
let singlecells_viz = function(rawdata, outputdir = "./") {
    ...
    for (let pkg in required_packages) {
    ...
    # 因此我们需要转置
    let input_matrix <- as.matrix(ec_data);
    ...
    # 创建 Seurat 对象
    let ec_seurat <- CreateSeuratObject(counts = input_matrix, project = "Genome_EC_Embedding", meta.data = meta_data);
    ...
    # ------------------------
    let p_dim <- DimPlot(ec_seurat, reduction = "umap", group.by = "Family", 
    ...
    ggsave(file.path(outputdir, "SC_01_UMAP_Family_Groups.png"), p_dim, width = 18, height = 8, dpi = 300);
    ...
    # ------------------------
    let top_ecs <- head(VariableFeatures(ec_seurat), 4);
    # 绘制 Feature Plot
    # min.cutoff 设置为 'q10' 可以去除底部 10% 的噪音，使信号更清晰
    let p_feature <- FeaturePlot(ec_seurat, features = top_ecs, 
                            min.cutoff = "q10", 
                            cols = c("lightgrey", "darkblue"), 
                            ncol = 2, pt.size = 1.5) + 
      plot_annotation(title = "Top Variable ECs Distribution");

    ggsave(file.path(outputdir, "SC_02_FeaturePlot_TopECs.png"), p_feature, width = 12, height = 10, dpi = 600);
    ...
    # 挑选前 10 个高变 EC 进行展示
    let marker_ecs <- head(VariableFeatures(ec_seurat), 30);
    let p_dot <- DotPlot(ec_seurat, features = marker_ecs, group.by = "Family") + 
      RotatedAxis() + 
    ...
    ggsave(file.path(outputdir,  "SC_03_DotPlot_Markers.png"), p_dot, width = 10, height = 18, dpi = 600);
    ...
    # ------------------------
    let p_vln <- VlnPlot(ec_seurat, features = marker_ecs[1:2], group.by = "Family", pt.size = 0.5, log = TRUE) + 
      NoLegend() +
      plot_annotation(title = "Expression Distribution of Key ECs");

    ggsave(file.path(outputdir, "SC_04_ViolinPlot.png"), p_vln, width = 30, height = 8, dpi = 300);
    ...
    # ------------------------
    let p_heat <- DoHeatmap(ec_seurat, features = marker_ecs, group.by = "Family", size = 3, angle = 45) + 
      NoLegend();

    ggsave(file.path(outputdir, "SC_05_Heatmap.png"), p_heat, width = 24, height = 9, dpi = 300);
